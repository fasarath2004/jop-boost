...
# Rate limiter: only call Gemini every N requests
_letter_gemini_counter = 0
_letter_counter_lock = threading.Lock()
LETTER_GEMINI_INTERVAL = int(os.environ.get("LETTER_GEMINI_INTERVAL", "20"))
...
async def generate_cover_letter(request: CoverLetterRequest):
    global _letter_gemini_counter
    client = get_genai_client()

    # Rate limit: only call Gemini every N requests
    with _letter_counter_lock:
        _letter_gemini_counter += 1
        should_use_gemini = (
            client is not None
            and _letter_gemini_counter % LETTER_GEMINI_INTERVAL == 0
        )

    if not should_use_gemini:
        logger.info(f"Generating offline cover letter (request #{_letter_gemini_counter}, Gemini every {LETTER_GEMINI_INTERVAL}).")
